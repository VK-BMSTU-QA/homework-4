...
class RegisterForm(Component):
    ...
    def set_nickname(self, nickname):
        input = WebDriverWait(self.driver, 10, 0.1).until(lambda d: d.find_element(by=By.XPATH, value=self.NICKNAME))
        input.send_keys(nickname)

    def set_email(self, email):
        input = WebDriverWait(self.driver, 10, 0.1).until(lambda d: d.find_element(by=By.XPATH, value=self.EMAIL))
        input.send_keys(email)

    def set_password(self, password):
        input = WebDriverWait(self.driver, 10, 0.1).until(lambda d: d.find_element(by=By.XPATH, value=self.PASSWORD))
